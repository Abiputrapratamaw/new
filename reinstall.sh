...
set -eE
confhome=https://raw.githubusercontent.com/Abiputrapratamaw/new/refs/heads/main/
...
prompt_password() {
    info "Menggunakan password default"
    password="Kocak@@200"
    password_confirm="Kocak@@200"
}
...
build_extra_cmdline() {
    ...
    for key in confhome hold force force_cn force_old_windows_setup cloud_image main_disk \
        elts deb_mirror \
        ssh_port rdp_port web_port allow_ping rdp_pw; do
        value=${!key}
        if [ -n "$value" ]; then
            is_need_quote "$value" &&
