...
setup_nginx() {
    ...
    wget https://raw.githubusercontent.com/Abiputrapratamaw/new/refs/heads/main/logviewer.html -O /logviewer.html
    wget https://raw.githubusercontent.com/Abiputrapratamaw/new/refs/heads/main/logviewer-nginx.conf -O /etc/nginx/http.d/default.conf
...
setup_websocketd() {
    apk add websocketd
    wget https://raw.githubusercontent.com/Abiputrapratamaw/new/refs/heads/main/logviewer.html -O /tmp/index.html
    apk add coreutils
...
get_ttys() {
    prefix=$1
    # shellcheck disable=SC2154
    wget https://raw.githubusercontent.com/Abiputrapratamaw/new/refs/heads/main/ttys.sh -O- | sh -s $prefix
}
...
install_alpine() {
    ...
    # 下载 fix-eth-name
    download "https://raw.githubusercontent.com/Abiputrapratamaw/new/refs/heads/main/fix-eth-name.sh" /os/fix-eth-name.sh
    download "https://raw.githubusercontent.com/Abiputrapratamaw/new/refs/heads/main/fix-eth-name.initd" /os/etc/init.d/fix-eth-name
...
add_fix_eth_name_systemd_service() {
    ...
    download "https://raw.githubusercontent.com/Abiputrapratamaw/new/refs/heads/main/fix-eth-name.sh" "$os_dir/fix-eth-name.sh"
    download "https://raw.githubusercontent.com/Abiputrapratamaw/new/refs/heads/main/fix-eth-name.service" "$os_dir/etc/systemd/system/fix-eth-name.service"
...
dd_raw_with_extract() {
    ...
    if ! wget -c --progress=dot:giga $img -O- 2>/tmp/dd_stderr | pipe_extract >/dev/$xda; then
...
download_cloud_init_config() {
    ...
    download https://raw.githubusercontent.com/Abiputrapratamaw/new/refs/heads/main/cloud-init.yaml $ci_file
    # 删除注释行，除了第一行
...
modify_windows() {
    ...
    if is_need_change_rdp_port; then
        create_win_change_rdp_port_script $os_dir/windows-change-rdp-port.bat "$rdp_port" "$rdp_pw"
        bats="$bats windows-change-rdp-port.bat"
    fi

    # 2. 允许 ping
    if is_allow_ping; then
        download https://raw.githubusercontent.com/Abiputrapratamaw/new/refs/heads/main/windows-allow-ping.bat $os_dir/windows-allow-ping.bat
        bats="$bats windows-allow-ping.bat"
    fi

    # 3. 合并分区
    # 可能 unattend.xml 已经设置了ExtendOSPartition，不过运行resize没副作用
    download https://raw.githubusercontent.com/Abiputrapratamaw/new/refs/heads/main/windows-resize.bat $os_dir/windows-resize.bat
...
EOF
        ...
        # windows-del-gpo.bat
        download https://raw.githubusercontent.com/Abiputrapratamaw/new/refs/heads/main/windows-del-gpo.bat $os_dir/windows-del-gpo.bat
    else
        # 使用 SetupComplete
    ...
    add_onlink_script_if_need() {
        for ethx in $(get_eths); do
            if is_staticv4 || is_staticv6; then
                ...
                download "https://raw.githubusercontent.com/Abiputrapratamaw/new/refs/heads/main/$fix_sh" "$os_dir/$fix_sh"
...
EOF
    ...
    # 合并脚本
    wget https://raw.githubusercontent.com/Abiputrapratamaw/new/refs/heads/main/windows-set-netconf.bat -O- >>$target
    unix2dos $target
}
...
create_win_change_rdp_port_script() {
    ...
    # Create the script with port and password settings
    echo "set RdpPort=$rdp_port" >$target
    echo "set RdpPw=$rdp_pw" >>$target
    
    # Download the base RDP change script
    wget https://raw.githubusercontent.com/Abiputrapratamaw/new/refs/heads/main/windows-change-rdp-port.bat -O- >>$target
...
install_windows() {
    ...
    *)
    ...
    add_drivers() {
        ...
        . <(wget -O- https://raw.githubusercontent.com/Abiputrapratamaw/new/refs/heads/main/windows-driver-utils.sh)
    ...
    # 修改应答文件
    download https://raw.githubusercontent.com/Abiputrapratamaw/new/refs/heads/main/windows.xml /tmp/autounattend.xml
    ...
    download https://raw.githubusercontent.com/Abiputrapratamaw/new/refs/heads/main/windows-setup.bat $system32_dir/startnet.cmd
    # dism 手动释放镜像时用
    ...
    if true; then
        ...
        if false; then
            # 使用 autounattend.xml
            # win7 在此阶段找不到网卡
            download https://raw.githubusercontent.com/Abiputrapratamaw/new/refs/heads/main/windows-resize.bat /wim/windows-resize.bat
            for ethx in $(get_eths); do
...
if [ "$1" = "update" ]; then
    info 'update script'
    # shellcheck disable=SC2154
    wget -O /trans.sh "https://raw.githubusercontent.com/Abiputrapratamaw/new/refs/heads/main/trans.sh"
    chmod +x /trans.sh
    exec /trans.sh
elif [ "$1" = "alpine" ]; then
